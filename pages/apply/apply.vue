<template>
	<view class="">
		...
		<view class="input-wrap">
			...
			<!-- {{applyForm}} -->
			<view class="input-box category"  @tap="visible.showSchoolPopup = true">
				<text class="input-name">院校</text>
				<input class="input-content" disabled placeholder="请选择院校" type="text" v-model="schoolName" />
				<text class="iconfont icon-youjiantou_huaban"></text>
			</view>
			
			<view class="input-box category"  @tap="onSubject">
				<text class="input-name">专业</text>
				<input class="input-content" disabled placeholder="请选择专业" type="text" v-model="subjectName" />
				<text class="iconfont icon-youjiantou_huaban"></text>
			</view>
			<!-- {{feeStandardList}} -->
			<view class="input-box category"  @tap="onFeeStandard">
				<text class="input-name">收费标准</text>
				<input class="input-content" disabled placeholder="请选择收费标准" type="text" v-model="feeStandardName" />
				<text class="iconfont icon-youjiantou_huaban"></text>
			</view>
		...
		<view class="instruction">
			<view>
				<u--textarea v-model="applyForm.MEMO" height="110" count maxlength="200" placeholder="您可以简要说明您的情况，以便为您提供更好的服务~" border="surround"></u--textarea>
			</view>
		</view>
		...
		<!-- 院校 -->
		<u-picker 
			:show="visible.showSchoolPopup"
			@close="visible.showSchoolPopup = false"
			@cancel="visible.showSchoolPopup = false"
			closeOnClickOverlay
			ref="schoolPicker"
			keyName="SCHOOL_NAME"
			confirmColor="#BC292D"
			:columns="schoolList"
			@confirm="(e) => selectConfirm(e, 'school')">
		</u-picker>
		<!-- 专业 -->
		<u-picker 
			:show="visible.showSubjectPopup"
			@close="visible.showSubjectPopup = false"
			@cancel="visible.showSubjectPopup = false"
			closeOnClickOverlay
			ref="subjectPicker"
			keyName="SUBJECT_NAME"
			confirmColor="#BC292D"
			:columns="subjectList"
			@confirm="(e) => selectConfirm(e, 'subject')">
		</u-picker>
		<!-- 收费标准 -->
		<u-picker 
			:show="visible.showeFeeStandardNamePopup"
			@close="visible.showeFeeStandardNamePopup = false"
			@cancel="visible.showeFeeStandardNamePopup = false"
			closeOnClickOverlay
			ref="feeStandardPicker"
			keyName="money"
			confirmColor="#BC292D"
			:columns="feeStandardList"
			@confirm="(e) => selectConfirm(e, 'feeStandard')">
		</u-picker>
...
export default {
	...
	data() {
		return {
			...
			visible: {
				...
				status: false,
				// -----
				showSchoolPopup: false, // 院校列表
				showSubjectPopup: false ,// 专业列表
				showeFeeStandardNamePopup: false // 收费标准
			},
			provinceColumns: [],
			cityColumns: [],
			schoolColumns: [],
	...
	onLoad(options) {
		...
		this.applyForm.USER_ID = this.userInfo.ID
		this.getSchoolList()
	...
	methods: {
		...
		getSubjectList() {
			this.$util.showLoading();
			const params = {
				SCHOOL_ID: this.applyForm.SCHOOL_ID
				// pageNum: 1,
				// pageSzie: 10
			};
			this.$http
				.get('school_subject_list', params)
				.then(res => {
					if (res.code === 1) {
		...
		getFeeStandardList() {
			...
			const params = {
				SCHOOL_ID: this.applyForm.SCHOOL_ID,
				SUBJECT_ID: this.applyForm.SUBJECT_ID
			};
			this.$http
				.get('get_fee_standard', params)
				.then(res => {
					if (res.code === 1) {
						res.data.money = (res.data.FEE_STANDARD_MONEY / 100) + '元'
		...
		selectConfirm (e, type) {
			console.log(e, type)
			if (type === 'school') {
				this.applyForm.SCHOOL_ID = e.value[0].ID
				this.schoolName = e.value[0].SCHOOL_NAME
				this.subjectName = ''
				this.getSubjectList()
				this.visible.showSchoolPopup = false
			} else if (type === 'subject') {
				this.applyForm.SUBJECT_ID = e.value[0].SUBJECT_ID
				this.subjectName = e.value[0].SUBJECT_NAME
				this.feeStandardName = ''
				this.getFeeStandardList()
				this.visible.showSubjectPopup = false
			} else {
				this.applyForm.FEE_STANDARD_ID = e.value[0].ID
				this.feeStandardName = e.value[0].money
				this.visible.showeFeeStandardNamePopup = false
			}
		},
		onSubject () {
			if (!this.applyForm.SCHOOL_ID) return this.$util.showToast('请选择院校')
			this.visible.showSubjectPopup = true
		},
		onFeeStandard () {
			if (!this.applyForm.SUBJECT_ID) return this.$util.showToast('请选择专业')
			this.visible.showeFeeStandardNamePopup = true
		},
		getintention () {
			this.$http
		...
		submit: debounce(function () {
			...
			this.isLoading = true
			const data = {
				"NAME": this.form.stuName,				"MOBILE": this.form.stuPhone,
				...this.applyForm
			}
			data.NAME = this.form.stuName
			data.MOBILE = this.form.stuPhone
			console.log(data)
